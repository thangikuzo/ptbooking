...
class PendingPTTab extends StatelessWidget {
  ...
  Future<void> _approvePT(BuildContext context, String uid) async {
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Đã duyệt thành PT chính thức!"), backgroundColor: Colors.green));
    }
  }
  ...
  Future<void> _rejectPT(BuildContext context, String uid) async {
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Đã từ chối hồ sơ."), backgroundColor: Colors.red));
    }
  }
